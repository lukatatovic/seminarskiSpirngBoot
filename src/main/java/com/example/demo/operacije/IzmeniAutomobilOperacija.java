...
import com.example.demo.exceptions.AutomobilNijePronadjenException;
...
@Service
public class IzmeniAutomobilOperacija implements GenerickaOperacija<Automobil, ResponseEntity> {
    @Autowired
    private AutomobilRepository automobilRepository;

    @Override
    public ResponseEntity<ResponseEntity> izvrsi(Automobil automobil) {
        Optional<Automobil> optionalAutomobil= automobilRepository.findById(automobil.getId());
        if(optionalAutomobil.isEmpty()){
            throw new AutomobilNijePronadjenException();
        }
